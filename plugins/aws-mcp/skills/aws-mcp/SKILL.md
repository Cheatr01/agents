...
If AWS MCP is not yet configured on the host, use `aws-mcp-setup` first. Do not install or authenticate an MCP server as part of an AWS operations request unless the user explicitly asks for setup.
